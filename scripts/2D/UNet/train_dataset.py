...
def test_model(model_path="/models/UNet/unet_pets.pth"):
    # Load model on MPS
    model = UNet(in_c=3, out_c=1).to(device)
    ...
    # Load test image
    test_images = os.listdir("/data/UNet/images")[:5]  # Test on first 5 images

    for img_name in test_images:
        img_path = os.path.join("/data/UNet/images", img_name)
        image = Image.open(img_path).convert("RGB")
